...
//			driver.executeScript("mobile:application:open", perfectoCommand);
     ...
     */
    protected byte[] downloadReport(final String type, RemoteWebDriver driver) {
...
//	}
    ...
    protected void scrollChecker(IOSDriver driver, String[] list) {
        for (int i = 0; i < list.length; i++) {
            ...
            WebElement me = driver.findElement(By.xpath("//UIAPickerWheel[" + (i + 1) + "]"));
            int mget = getMonthInt(me.getText().split(",")[0]);

            if (i == 0) {
                scrollAndSearch(driver, list[i], me, mget > getMonthInt(list[i]));
            } else {
                scrollAndSearch(driver, list[i], me, Integer.parseInt(me.getText().split(",")[0]) > Integer.parseInt(list[i]));
            }
        }
    }
    ...
    @SuppressWarnings("rawtypes")
    private void scrollAndSearch(IOSDriver driver, String value, WebElement me, Boolean direction) {
        String x = getLocationX(me);
        String y = getLocationY(me);
        while (!me.getText().contains(value)) {
